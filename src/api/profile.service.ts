...
const ProfileService = {
    ...
    getKabupaten: async function(parameter) {
        try {
            return await ApiService.fetch(ApiService.config.apiUrl.resources.masterKabupaten.path+'?'+qs(parameter, false));
        } catch (error) {
            const msg = (error as any);
            throw msg;
        }
